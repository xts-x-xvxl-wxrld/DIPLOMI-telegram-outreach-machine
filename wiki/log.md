...
## [2026-04-28] fix | Add engagement join debug visibility

- Investigated missing engagement joins locally and found two separate causes:
  stale Docker-packaged runtime files on this machine and silent task-first
  `community.join` enqueue failures in the backend confirm path.
- Added structured `community.join` worker logs for start, skip reasons,
  account acquisition, join result, and unexpected failures so the next repro
  is visible in worker logs without the bot UI.
- Changed task-first confirmation to log and return a blocking error when the
  join job cannot be queued instead of reporting a misleading success state.
- Updated the bot wizard success card to use the backend confirmation message
  and added a regression test for join-enqueue failure handling.

## [2026-04-28] fix | Promote task-first communities before join

- Reproduced the real task-first confirm flow against the local Docker stack
  after rebuilding containers and verifying Postgres was on Alembic head
  `20260428_0015`.
- Confirmed the earlier bugfix worked: task-first confirm now enqueues
  `community.join` instead of silently pretending success when enqueueing fails.
- Found the next join blocker in live DB and RQ state: a resolved engagement
  target could be approved while its backing `communities.status` stayed
  `candidate`, so the join worker finished with
  `{'status': 'skipped', 'reason': 'community_not_approved'}` and wrote no
  `community_account_memberships` or `engagement_actions` rows.
- Updated task-first confirmation to promote resolved target communities to
  `approved` with `reviewed_at` before enqueueing `community.join`, and added a
  regression test for that status transition.
- Re-ran the live flow through `/api/engagements/{id}/wizard-confirm` using a
  direct-community target and confirmed the join job finished as
  `{'status': 'processed', 'action_status': 'sent'}` with a `joined`
  membership row and a `join` audit row in `engagement_actions`.
- Also noted a separate follow-up outside the join fix: the manual
  `engagement.detect` job triggered during task-first confirm still reports
  `{'status': 'skipped', 'reason': 'engagement_disabled'}` because it reads the
  legacy community-level engagement settings path rather than task-first
  engagement settings.

## [2026-04-28] chore | Stop tracking generated egg-info metadata

- Removed the committed `telegram_community_discovery.egg-info/` packaging
  metadata files from the repo because they are generated by local packaging
  and install steps rather than maintained source files.
- Added `*.egg-info/` to `.gitignore` so editable installs do not keep
  re-dirtying the worktree with packaging metadata churn.
