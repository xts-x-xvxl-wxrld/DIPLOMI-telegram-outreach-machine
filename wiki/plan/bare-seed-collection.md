...
CSV upload
  ...
  -> collection.run for those seed communities
...
collect the exact imported communities before trying to discover adjacent communities.

## Scope

- Keep CSV seed import unchanged.
- Keep `seed.resolve` as the step that maps public Telegram usernames to `communities`.
- After a seed resolves, queue collection for the resolved seed community.
- Implement a bare `collection.run` worker that fetches Telegram metadata and visible members.
- Persist users and community membership safely without phone numbers or person-level scores.
- Mark analysis as skipped for this bare member import path unless message collection is added later.
...
- Collection failures should create failed `collection_runs` rows instead of partially pretending success.

## Implementation Steps

1. Add collection persistence service for metadata snapshots and visible members.
2. Add Telethon collection adapter for full community info and participant iteration.
3. Wire `collection.run` dispatch to the real worker.
4. Queue initial collection jobs for communities resolved by `seed.resolve`.
