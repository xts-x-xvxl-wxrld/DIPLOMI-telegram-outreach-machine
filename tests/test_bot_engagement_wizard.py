...
class _FakeWizardApiClient(_FakeApiClient):
    def __init__(self) -> None:
        ...
        self.engagement_detail: dict[str, Any] = {
            "engagement_id": _ENG_NEW_ID,
            "target_label": "Founder Circle",
            "topic_label": "Open CRM",
            "account_label": "@engagement_bot",
    ...
    async def wizard_confirm_engagement(self, engagement_id, *, requested_by=None):
        ...
        return {
            ...
            "next_callback": f"eng:det:open:{engagement_id}",
            "engagement_id": engagement_id,
        }
...
async def test_wizard_cancel_shows_confirmation_prompt() -> None:
    ...
    assert f"eng:wz:cancel_no:{_ENG_NEW_ID}" in callbacks
...
async def test_wizard_cancel_yes_clears_pending() -> None:
    ...
    assert context.application.bot_data[CONFIG_EDIT_STORE_KEY].get(123) is None
    edit_text = cancel_yes_update.callback_query.edits[0]["text"]
    edit_markup = cancel_yes_update.callback_query.edits[0]["reply_markup"]
    callbacks = [button.callback_data for row in edit_markup.inline_keyboard for button in row]
    assert "Engagements" in edit_text
    assert "op:add" in callbacks
    assert "op:engs" in callbacks

@pytest.mark.asyncio
async def test_wizard_cancel_back_restores_current_step() -> None:
    client = _FakeWizardApiClient()
    context = _wiz_context(client)

    await _wizard_through_step2(context)

    cancel_back_update = _callback_update(f"eng:wz:cancel_no:{_ENG_NEW_ID}")
    await callback_query(cancel_back_update, context)

    reply_text = cancel_back_update.callback_query.message.replies[0]["text"]
    reply_markup = cancel_back_update.callback_query.message.replies[0]["reply_markup"]
    callbacks = [button.callback_data for row in reply_markup.inline_keyboard for button in row]
    assert "Step 2 of 5" in reply_text
    assert f"eng:wz:tp:{_C_TOPIC_1}:{_C_ENG_NEW}" in callbacks
    assert f"eng:wz:cancel:{_ENG_NEW_ID}" in callbacks
