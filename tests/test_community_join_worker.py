...
async def test_community_join_records_success_and_releases_account() -> None:
    ...
    result = await process_community_join(
        ...
        enqueue_collection_fn=_capture_collection(collection_calls),
    )
...
def _capture_collection(calls: list[dict[str, object]]):
    def fake_collection(community_id: object, *, reason: str, requested_by: str) -> None:
        calls.append(
            {
                "community_id": community_id,
                "reason": reason,
